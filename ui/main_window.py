...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.rulebook_dir_action = QAction("规则书路径（Excel）...", self)
        self.rulebook_dir_action.triggered.connect(self._set_rulebook_dir)
        self.more_menu.addAction(self.rulebook_dir_action)
        self.rulebook_pdf_action = QAction("打开规则书（PDF）...", self)
        self.rulebook_pdf_action.setShortcut("Ctrl+O")
        self.rulebook_pdf_action.triggered.connect(self._open_rulebook_pdf)
        self.addAction(self.rulebook_pdf_action)
        self.more_menu.addAction(self.rulebook_pdf_action)
        self.more_menu.addSeparator()
        self.bg_action = QAction("设置背景图片...", self)
        self.bg_action.setShortcut("Ctrl+Shift+B")
        self.bg_action.triggered.connect(self._set_background)
        self.addAction(self.bg_action)
        self.more_menu.addAction(self.bg_action)
        self.opacity_action = QAction("背景水印强度...", self)
        self.opacity_action.triggered.connect(self._set_watermark)
        self.more_menu.addAction(self.opacity_action)
        self.clear_action = QAction("清除背景图片", self)
        self.clear_action.triggered.connect(self._clear_background)
        self.more_menu.addAction(self.clear_action)
        self.more_menu.addSeparator()
        self.export_action = QAction("导出战斗日志...", self)
        self.export_action.setShortcut("Ctrl+S")
        self.export_action.triggered.connect(self._export_log)
